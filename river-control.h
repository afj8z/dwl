...
setborderpx (const Arg *arg)
{
    ...
    Monitor *m;
    borderpx = arg->ui;
    wl_list_for_each (c, &clients, link) { c->bw = borderpx; }
    wl_list_for_each (m, &mons, link) { arrange (m); }
}

void
setfocuscolor (const Arg *arg)
{
...
setcornerradius (const Arg *arg)
{
    ...
    wl_list_for_each (c, &clients, link)
    {
        ...
        // Manually update scene node to ensure changes apply even if radius is
        // 0
        if (c->round_border)
            {
    ...
    {
        c->corner_radius = corner_radius;
        /* This function handles both the border rect AND the inner buffer
         * clipping
         */
...
void
zriver_control_destroy (struct wl_client *client, struct wl_resource *resource)
{
    printf ("destroy!\n");
}
void
clear_str_store (struct wl_list *str_store)
{
...
void
free_str_store (struct Str_link *str_link)
{
    int i;
...
char *
append_str_store (char **str_store_array, const char *string, int index)
{
...
struct Str_link *
add_arg_str_store (const char *string)
{
    struct Str_link *str_link = calloc (1, sizeof (struct Str_link));
    int i;
    int string_len = strlen (string) + 1;
            ...
            if (rl != NULL)
                {
                    ...
                    rl->no_remove = true; /* remove this line to make rules[]
                                             removed by clear-rules */
...
void
create_mode_user (const Arg *arg)
{
...
#ifdef KEYS_USED
void
default_binds (struct wl_list *keys_list)
{
    Key *k;
    Key_linked *kl;
...
void
zriver_control_add_argument (struct wl_client *client,
                             struct wl_resource *resource,
                             const char *argument)
{
    struct zriver_arg_list_resource *args
    ...
    if (args->argc == 0)
        {
            if (strcmp ("rule-add", argument) == 0)
            ...
            else if (strcmp ("map", argument) == 0
                     || strcmp ("bind", argument) == 0)
                {
                ...
                {
                    for (fst = Func_str_type_pair_list;
                         fst < END (Func_str_type_pair_list); fst++)
                        {
                            if (strcmp (argument, fst->func_str) == 0)
                                {
                                    ...
                                    args->p.fa = calloc (
                                        1,
                                        sizeof (struct zriver_func_arg_pair));
                                    if (args->p.fa == NULL)
                                        {
                                            args->error = true;
                                            args->error_msg
                    ...
                    if (args->error != true
                        && args->type != ZRIVER_ARG_TYPE_FUNC)
                        {
                            args->error = true;
                            args->error_msg
                                = zriver_error_no_matching_argument;
                        }
                }
        }
            ...
            switch (args->rule_match_type)
                ...
                case (ZRIVER_RULE_MATCH_TYPE_APPID):
                    if (args->p.rl->rule->id == NULL)
                        {
                            args->p.rl->rule->id
                                = append_str_store (args->str_link->string,
                                                    argument, args->argc - 1);
                            args->rule_match_type
                                = ZRIVER_RULE_MATCH_TYPE_NONE;
                        }
                    else
                        {
                            args->error = true;
    ...
    else if (args->type == ZRIVER_ARG_TYPE_KEY)
        {
            if (args->argc == 2)
                {
                    for (ms = Mod_str_pair_list; ms < END (Mod_str_pair_list);
                         ms++)
                        {
            ...
            else if (args->argc == 3)
                {
                    if (strcmp (argument, "none") == 0)
                        ...
                        {
                            args->p.kl->key->keysym = xkb_keysym_from_name (
                                argument, XKB_KEYSYM_NO_FLAGS);
                            if (args->p.kl->key->keysym == XKB_KEY_NoSymbol)
                                {
                                    args->error = true;
                                    args->error_msg
                                        = zriver_error_invalid_keysym;
                                }
                        }
                }
            else if (args->argc == 4)
            ...
            else if (args->argc > 5 && args->argc < STR_LINK_ARRAY_SIZE
                     && args->key_arg_type == FUNC_STR_ARG_TYPE_STRING_ARRAY
                     && args->p.kl->key->arg.v != NULL)
                {
                    append_str_store ((char **)args->p.kl->key->arg.v,
                                      argument, args->argc - 5);
                }
        }
...
void
zriver_control_run_command (struct wl_client *client,
                            struct wl_resource *resource,
                            ...
                            uint32_t callback)
{
    ...
    struct wl_resource *callback_interface = wl_resource_create (
        client, &zriver_command_callback_v1_interface,
        zriver_command_callback_v1_interface.version, callback);
    if (args->argc == 0)
        {
            zriver_command_callback_v1_send_failure (
                callback_interface, zriver_error_too_few_args);
        }
    else if (args->error == true)
        {
            ...
            zriver_command_callback_v1_send_failure (callback_interface,
                                                     args->error_msg);
        }
    else if (args->error == false)
        {
            if (args->type == ZRIVER_ARG_TYPE_KEY)
                {
                    if (args->p.kl->key != NULL
                        && args->p.kl->key->func != NULL)
                        {
                            wl_list_insert (&args->key_mode->linked_keys,
                                            &args->p.kl->link);
                            zriver_command_callback_v1_send_success (
                                callback_interface, "bind success!");
                        }
                    else
                        {
                            if (args->str_link != NULL)
                    ...
                    if (args->rule_valid == true)
                        {
                            ...
                            if (args->p.rl->rule->title != NULL
                                || args->p.rl->rule->id != NULL)
                                {
                                    ...
                                    {
                                        r = rl->rule;
#define CHECKNULL(a, b)                                                       \
    ((a != NULL && b != NULL && strcmp (a, b) == 0)                           \
     || (a == NULL && b == NULL))
                                        if (CHECKNULL (args->p.rl->rule->title,
                                                       r->title)
                                            && CHECKNULL (args->p.rl->rule->id,
                                                          r->id))
                                            {
                                                wl_list_remove (&rl->link);
                                                if (rl->no_free_rule == false)
...
#endif
                            wl_list_insert (rules_list.prev,
                                            &args->p.rl->link);
                            if (replaced_rule == true)
                                {
                                    zriver_command_callback_v1_send_success (
                                        callback_interface, "rule replaced!");
                                }
                            else
                                {
                                    zriver_command_callback_v1_send_success (
                                        callback_interface, "rule success!");
                                }
                        }
                    else
                        {
...
static void
zriver_control_handle_destory (struct wl_resource *resource)
{
    struct zriver_arg_list_resource *zriver_arg_list_resource
        = wl_resource_get_user_data (resource);
    free (zriver_arg_list_resource);
    printf ("handle destroy\n");
}
static void
zriver_control_handle_bind (struct wl_client *client, void *data,
                            uint32_t version, uint32_t id)
{
    struct zriver_arg_list_resource *zriver_arg_list_resource
        = calloc (1, sizeof (struct zriver_arg_list_resource));
    struct wl_resource *resource
        = wl_resource_create (client, &zriver_control_v1_interface,
                              zriver_control_v1_interface.version, id);
    zriver_arg_list_resource->error_msg = zriver_error_generic;

    wl_resource_set_implementation (resource, &zriver_control_interface,
                                    zriver_arg_list_resource,
