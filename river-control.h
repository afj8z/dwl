...
void setup_rules(void) {
  ...
  for (r = rules; r < END(rules); r++) {
    ...
    if (rl != NULL) {
      ...
      rl->no_remove =
          true; /* remove this line to make rules[] removed by clear-rules */
...
void zriver_control_add_argument(struct wl_client *client,
  ...
  if (args->argc == 0) {
    if (strcmp("rule-add", argument) == 0) {
      ...
      for (fst = Func_str_type_pair_list; fst < END(Func_str_type_pair_list);
           fst++) {
    ...
    case (ZRIVER_RULE_MATCH_TYPE_APPID):
      if (args->p.rl->rule->id == NULL) {
        args->p.rl->rule->id =
            append_str_store(args->str_link->string, argument, args->argc - 1);
    ...
    if (args->argc == 2) {
      ...
      if (strcmp(argument, "none") == 0) {
        ...
        args->p.kl->key->keysym =
            xkb_keysym_from_name(argument, XKB_KEYSYM_NO_FLAGS);
      ...
      arg = &args->p.kl->key->arg;
               ...
               args->key_arg_type == FUNC_STR_ARG_TYPE_STRING_ARRAY &&
               args->p.kl->key->arg.v != NULL) {
      append_str_store((char **)args->p.kl->key->arg.v, argument,
                       args->argc - 5);
    }
  }
...
#define CHECKNULL(a, b)                                                        \
  ((a != NULL && b != NULL && strcmp(a, b) == 0) || (a == NULL && b == NULL))
            if (CHECKNULL(args->p.rl->rule->title, r->title) &&
                CHECKNULL(args->p.rl->rule->id, r->id)) {
              wl_list_remove(&rl->link);
              if (rl->no_free_rule == false) {
                free(rl->rule);
              }
