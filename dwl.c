...
static void iter_xdg_scene_buffers_blur (struct wlr_scene_buffer *buffer,
                                         int sx, int sy, void *user_data);
static void scenebuffersetopacity (struct wlr_scene_buffer *buffer, int sx,
                                   int sy, void *user_data);
static void
...
cleanupmon (struct wl_listener *listener, void *data)
{
    ...
    wl_list_for_each_safe (ipc_output, ipc_output_tmp, &m->dwl_ipc_outputs,
                           link) wl_resource_destroy (ipc_output->resource);
...
static void
iter_layer_scene_buffers_blur (struct wlr_scene_buffer *buffer, int sx, int sy,
                               void *user_data)
{
    if (!blur)
        return;
    ...
    if (c->surface.xdg->initial_commit)
        {
            ...
            setmon (c, NULL,
                    0); /* Make sure to reapply rules in mapnotify() */
...
createmon (struct wl_listener *listener, void *data)
{
    ...
    if (blur)
        {
            m->blur_layer
                = wlr_scene_optimized_blur_create (&scene->tree, 0, 0);
                      ...
                      uint32_t id)
{
    struct wl_resource *manager_resource = wl_resource_create (
        client, &zdwl_ipc_manager_v2_interface, version, id);
    if (!manager_resource)
        {
...
void
dwl_ipc_manager_get_output (struct wl_client *client,
                            ...
                            struct wl_resource *output)
{
    ...
    wl_resource_set_implementation (output_resource,
                                    &dwl_output_implementation, ipc_output,
...
void
dwl_ipc_manager_release (struct wl_client *client,
                         struct wl_resource *resource)
{
...
dwl_ipc_output_printstatus_to (DwlIpcOutput *ipc_output)
{
    ...
    if (wl_resource_get_version (ipc_output->resource)
        ...
        {
            zdwl_ipc_output_v2_send_fullscreen (
                ipc_output->resource, focused ? focused->isfullscreen : 0);
        }
    if (wl_resource_get_version (ipc_output->resource)
        >= ZDWL_IPC_OUTPUT_V2_FLOATING_SINCE_VERSION)
        {
            zdwl_ipc_output_v2_send_floating (
                ipc_output->resource, focused ? focused->isfloating : 0);
        }
...
void
dwl_ipc_output_set_layout (struct wl_client *client,
...
void
dwl_ipc_output_set_tags (struct wl_client *client,
    ...
    if (!client_is_x11 (c))
        {
            ...
            if (shadow)
                {
                    c->shadow = wlr_scene_shadow_create (
                        c->scene, 0, 0, c->corner_radius, shadow_blur_sigma,
                        shadow_color);
    ...
    if (corner_radius > 0 && c->round_border)
        {
            ...
            wlr_scene_rect_set_size (c->round_border, c->geom.width,
                                     c->geom.height);
...
void
scenebuffersetopacity (struct wlr_scene_buffer *buffer, int sx, int sy,
                       void *data)
{
    Client *c = data;
...
setopacityunfocus (const Arg *arg)
{
    ...
    wlr_scene_node_for_each_buffer (&sel->scene_surface->node,
                                    scenebuffersetopacity, sel);
}
...
tile (Monitor *m)
{
    ...
    if (n > m->nmaster)
        mw = m->nmaster
                 ? (int)roundf ((m->w.width + m->gappiv * ie) * m->mfact)
                 : 0;
    else
...
void
iter_xdg_scene_buffers_blur (struct wlr_scene_buffer *buffer, int sx, int sy,
                             void *user_data)
{
    Client *c = user_data;
    struct wlr_scene_surface *scene_surface
...
// void
// iter_xdg_scene_buffers_opacity (struct wlr_scene_buffer *buffer, int sx,
//                                 int sy, void *user_data)
// {
//     Client *c = user_data;
//     struct wlr_scene_surface *scene_surface
...
//         = wlr_xdg_surface_try_from_wlr_surface (scene_surface->surface);
//
//     if (c && xdg_surface && xdg_surface->role ==
...
client_set_shadow_blur_sigma (Client *c, int blur_sigma)
{
    ...
    wlr_scene_shadow_set_clipped_region (
        c->shadow,
        (struct clipped_region){ .corner_radius = c->corner_radius + c->bw,
                                 .corners = CORNER_LOCATION_ALL,
                                 .area = { blur_sigma, blur_sigma,
    ...
    if (shadow && c->shadow)
        {
            client_set_shadow_blur_sigma (
                c, (int)round (focused ? shadow_blur_sigma_focus
                                       : shadow_blur_sigma));
            if (c->has_shadow_enabled)
                {
                    wlr_scene_shadow_set_color (c->shadow,
                                                focused ? shadow_color_focus
                                                        : shadow_color);
                }
        }
}
